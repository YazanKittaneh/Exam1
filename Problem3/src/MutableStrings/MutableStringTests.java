...
public class MutableStringTests
{
  ...
  */
  public void checkChars(String msg, String str, MutableString mut)
  {
    if (str.length() != mut.length())
      {
        fail(msg + " (strings have different lengths): " + str.length()
             + " vs. " + mut.length());
    ...
    for (int i = 0; i < str.length(); i++)
      {
        if (str.charAt(i) != mut.charAt(i))
          {
            fail(msg + " (mismatch at char " + i + "): " + "'" + str.charAt(i)
                 + "' vs '" + mut.charAt(i) + "'");
  ...
  */
  public void testReplace1(String str)
  {
    Random gen = new Random();
    String tmp = str;
    MutableString mut = new BasicMutableString(str);
        ...
        if (!tmp.equals(mut.toString()))
          {
            ...
            fail(save + ".replace(" + quote(pattern) + ", "
                 + quote(replacement) + ")" + " should yield " + quote(tmp)
                 + " but gave " + quote(mut.toString()));
  ...
  */
  ...
  public void testReplace()
  {
    for (int i = 0; i < 10; i++)
      {
        testReplace1("aaaaaabbbbbbb");
        testReplace1("alphabetalphabetalphabet");
      } // for
  } // testReplace
